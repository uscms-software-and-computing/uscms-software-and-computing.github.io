...
which markdown files were updated during Q4 2025 (Oct 1 – Dec 31, 2025).
...
def build_pdf(results, repo_slug, base_path, output_path):
    ...
    sub_style   = ParagraphStyle("sub",   parent=styles["Normal"], fontSize=8, textColor=colors.grey, spaceAfter=12)
    story.append(Paragraph("Postdoc Research Update – Freshness Report", title_style))
    story.append(Paragraph(
        f"Repo: {repo_slug} &nbsp;|&nbsp; Path: {base_path} &nbsp;|&nbsp; "
        f"Up-to-date: Oct 1, 2025 or later &nbsp;|&nbsp; Generated: {datetime.now().strftime('%Y-%m-%d %H:%M UTC')}",
        sub_style
    ))

    # Summary counts
    total      = len(results)
    up_to_date = sum(1 for r in results if r["in_q4"])
    outdated   = sum(1 for r in results if not r["in_q4"] and r["total_commits"] > 0)
    no_commits = sum(1 for r in results if r["total_commits"] == 0)

    summary_data = [["Total Files", "Up-to-date (Q4 2025+)", "Outdated", "No Commits"],
    ...
    # Main table
    url_style = ParagraphStyle("url", parent=styles["Normal"], fontSize=7, textColor=colors.HexColor("#3b82f6"))
    file_style = ParagraphStyle("file", parent=styles["Normal"], fontSize=7, fontName="Courier")

    header = ["#", "File", "Status", "Last Commit", "Q4 Commits", "Total", "Page URL", "Commits URL"]
    col_widths = [0.3*inch, 2.2*inch, 0.9*inch, 0.85*inch, 0.7*inch, 0.5*inch, 2.1*inch, 2.1*inch]
    ...
    for i, r in enumerate(results, 1):
        ...
        table_data.append([
            ...
            status,
            last,
            str(r["q4_commits"]) if r["q4_commits"] else "—",
...
def get_permalink(filepath, token=None):
    """Fetch file content from GitHub and extract permalink from YAML front matter.
    Falls back to filename-derived URL if no permalink is found."""
    import base64, re
...
def get_file_commits(filepath, token=None):
    ...
    commits = list(paginate(url, token))
    return commits


def parse_date(date_str):
    """Parse ISO 8601 date string to timezone-aware datetime."""
    # GitHub returns e.g. "2025-11-03T14:22:00Z"
    return datetime.fromisoformat(date_str.replace("Z", "+00:00"))


def classify(commits):
    """
    Given a file's commit list, return:
      - last_commit_date  (datetime or None)
      - in_q4             (bool)
      - q4_commit_count   (int)
      - all_time_commits  (int)
    """
    if not commits:
    ...
    last = dates[0]
    q4_dates = [d for d in dates if d >= Q4_START]
    return last, bool(q4_dates), len(q4_dates), len(dates)
...
<title>Postdoc Update Freshness Report – Q4 2025</title>
<style>
...
<p class="subtitle">Repository: <strong>{repo}</strong> &nbsp;|&nbsp; Path: <code>{path}</code> &nbsp;|&nbsp;
  Up-to-date threshold: <strong>Oct 1, 2025 or later</strong> &nbsp;|&nbsp; Generated: <strong>{generated}</strong></p>

<div class="summary">
  <div class="stat"><div class="num">{total}</div><div class="label">Total files</div></div>
  <div class="stat"><div class="num green">{up_to_date}</div><div class="label">Up-to-date (Q4 2025)</div></div>
  <div class="stat"><div class="num red">{outdated}</div><div class="label">Not updated in Q4</div></div>
...
def build_html(results, repo_slug, base_path):
    total     = len(results)
    up_to_date = sum(1 for r in results if r["in_q4"])
    outdated   = sum(1 for r in results if not r["in_q4"] and r["total_commits"] > 0)
    no_commits = sum(1 for r in results if r["total_commits"] == 0)
    ...
    for i, r in enumerate(results, 1):
        if r["in_q4"]:
            badge_class, status = "badge-up-to-date", "✅ Up-to-date"
        elif r["total_commits"] == 0:
            badge_class, status = "badge-no-commits", "⚠️ No commits"
        else:
            badge_class, status = "badge-outdated", "❌ Outdated"
        ...
        rows.append(ROW_TEMPLATE.format(
            ...
            last_commit=last, q4_count=r["q4_commits"] or "—",
            total_count=r["total_commits"] or "—", gh_url=gh_url, web_url=web_url
        ))

    return HTML_TEMPLATE.format(
        repo=repo_slug, path=base_path,
...
def main():
    ...
    parser.add_argument("--output", default="postdoc_freshness_report.html",
                        help="Output HTML file (default: postdoc_freshness_report.html)")
    ...
    for idx, filepath in enumerate(md_files, 1):
        ...
        last_date, in_q4, q4_count, total = classify(commits)
        marker = "✅" if in_q4 else ("⚠️ " if total == 0 else "❌")
        print(f"  {marker}  last: {last_date.strftime('%Y-%m-%d') if last_date else 'n/a'}"
              f"  q4_commits: {q4_count}  total: {total}")
        results.append({
            "path":             filepath,
            "permalink":        permalink,
    ...
    # Sort: up-to-date first, then by last commit desc
    results.sort(key=lambda r: (not r["in_q4"], -(r["last_commit_date"].timestamp() if r["last_commit_date"] else 0)))

    # Summary
    up_to_date = sum(1 for r in results if r["in_q4"])
    print(f"\n{'='*55}")
    print(f"  Total files : {len(results)}")
    print(f"  ✅ Up-to-date (Q4 2025) : {up_to_date}")
    print(f"  ❌ Outdated             : {sum(1 for r in results if not r['in_q4'] and r['total_commits']>0)}")
    print(f"  ⚠️  No commits found    : {sum(1 for r in results if r['total_commits']==0)}")
    print(f"{'='*55}\n")

    # Write HTML
    html = build_html(results, repo_slug, BASE_PATH)
    ...
    if args.pdf:
        ...
        build_pdf(results, repo_slug, BASE_PATH, pdf_path)
